...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        await db.execute("CREATE INDEX IF NOT EXISTS idx_sh_user ON search_history(user_id)")
...
async def get_stats() -> dict:
    async with aiosqlite.connect(DB_PATH) as db:
        async def _c(sql, *a):
            async with db.execute(sql, a) as cur:
                return (await cur.fetchone())[0]
        today = date.today().isoformat()
        return {
            "total_users":     await _c("SELECT COUNT(*) FROM users"),
            "active_users":    await _c("SELECT COUNT(*) FROM users WHERE is_banned=0"),
            "banned_users":    await _c("SELECT COUNT(*) FROM users WHERE is_banned=1"),
            "total_downloads": await _c("SELECT COUNT(*) FROM downloads"),
            "audio_downloads": await _c("SELECT COUNT(*) FROM downloads WHERE media_type='audio'"),
            "video_downloads": await _c("SELECT COUNT(*) FROM downloads WHERE media_type='video'"),
            "today_downloads": await _c("SELECT COUNT(*) FROM downloads WHERE downloaded_at LIKE ?", f"{today}%"),
            "today_users":     await _c("SELECT COUNT(*) FROM users WHERE last_active LIKE ?", f"{today}%"),
            "total_searches":  await _c("SELECT COUNT(*) FROM search_history"),
            "total_favorites": await _c("SELECT COUNT(*) FROM favorites"),
            "total_admins":    await _c("SELECT COUNT(*) FROM admins"),
        }


async def check_daily_limit(user_id: int, limit: int = 30):
    today = date.today().isoformat()
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT dl_today, dl_date FROM users WHERE user_id=?", (user_id,)
        ) as c:
            row = await c.fetchone()
        if not row:
            return True, limit
        ...
        if dl_date != today:
            await db.execute(
                "UPDATE users SET dl_today=0, dl_date=? WHERE user_id=?", (today, user_id))
            await db.commit()
            return True, limit
        if dl_today >= limit:
            return False, 0
...
async def increment_daily_download(user_id: int):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET dl_today=CASE WHEN dl_date=? THEN dl_today+1 ELSE 1 END, "
            "dl_date=? WHERE user_id=?",
            (today, today, user_id))
        await db.commit()


async def add_search_history(user_id: int, query: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def get_search_history(user_id: int, limit: int = 10):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT DISTINCT query, searched_at FROM search_history "
...
async def add_favorite(user_id: int, title: str, artist: str, url: str) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT id FROM favorites WHERE user_id=? AND url=?", (user_id, url)
        ) as c:
            if await c.fetchone():
                return False
        await db.execute(
            "INSERT INTO favorites (user_id, title, artist, url) VALUES (?, ?, ?, ?)",
...
async def remove_favorite(fav_id: int, user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM favorites WHERE id=? AND user_id=?", (fav_id, user_id))
        await db.commit()


async def log_download(user_id: int, title: str, platform: str, media_type: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO downloads (user_id, title, platform, media_type) VALUES (?, ?, ?, ?)",
...
async def add_admin(user_id: int, added_by: int = 0):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def log_error(user_id: int, error_text: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
        ...
        await db.execute(
            ...
            "(SELECT id FROM error_logs ORDER BY logged_at DESC LIMIT 200)")
        await db.commit()
